      ...
      <div className="bg-surface-card rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-5 border-b border-border-subtle flex items-center justify-between bg-surface-bg">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-text-primary text-base">{t('dashboard.transaction_details')}</h3>
                <span className="text-[10px] font-semibold text-text-secondary bg-stone-200/80 px-2 py-0.5 rounded">
                  #TXN-{transaction.id}
                </span>
              </div>
              <p className="text-xs text-text-secondary">{t('dashboard.direct_market_txn')}</p>
            </div>
          </div>
          <button 
            type="button" 
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-agrigreen-500/10 border border-agrigreen-500/30 rounded-2xl p-4 flex items-center justify-between">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-agrigreen-900 block">{t("transaction.agreed_total")}</span>
              <div className="text-2xl font-bold text-agrigreen-900 mt-0.5">
                ₹{transaction.total_amount.toLocaleString('en-IN')}
              </div>
            </div>
            <div className="text-right">
              <span className="text-xs font-semibold text-text-secondary block">{t('dashboard.demo_payment_tracking')}</span>
              <span className={`inline-block mt-1 text-xs font-bold px-3 py-1 rounded-full border ${
                ...
                {t('dashboard.payment')}: {t(`dashboard.${transaction.payment_status.toLowerCase()}`)}
              </span>
            </div>
          </div>
          ...
          <div className="bg-surface-bg p-5 rounded-2xl border border-border-subtle space-y-3">
            <h4 className="text-xs font-semibold uppercase text-text-secondary tracking-wider">{t("transaction.timeline")}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 text-center text-xs pt-1">
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="font-bold text-text-primary text-[11px]">{t('dashboard.offer_accepted')}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="font-bold text-text-primary text-[11px]">{t('dashboard.confirmed')}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className={`text-[11px] ${isStep3Done ? 'font-bold text-text-primary' : 'text-text-secondary'}`}>{t("transaction.processing")}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className={`text-[11px] ${isStep4Done ? 'font-bold text-text-primary' : 'text-text-secondary'}`}>{t("transaction.paid")}</span>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <span className={`text-[11px] ${isStep5Done ? 'font-bold text-text-primary' : 'text-text-secondary'}`}>{t("transaction.completed")}</span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-xs text-text-primary">
            <div className="bg-surface-bg p-4 rounded-xl border border-border-subtle space-y-1">
              <span className="text-text-secondary font-medium uppercase text-[10px]">{t("transaction.crop_quality")}</span>
              <div className="font-bold text-text-primary text-sm">{translateCrop(transaction.crop_name, t)}</div>
              <div className="text-text-secondary font-medium">{t(`quality.${transaction.quality_grade}`)}</div>
            </div>

            <div className="bg-surface-bg p-4 rounded-xl border border-border-subtle space-y-1">
              <span className="text-text-secondary font-medium uppercase text-[10px]">{t("buyer.quantity")}</span>
            ...
            <div className="bg-surface-bg p-4 rounded-xl border border-border-subtle space-y-1">
              <span className="text-text-secondary font-medium uppercase text-[10px]">{t("buyer.agreed_price")}</span>
              <div className="font-bold text-agrigreen-900 text-sm">₹{transaction.agreed_price_per_quintal.toLocaleString('en-IN')} / {t("units.qtl")}</div>
            </div>

            <div className="bg-surface-bg p-4 rounded-xl border border-border-subtle space-y-1">
              <span className="text-text-secondary font-medium uppercase text-[10px]">{t("common.location")}</span>
              <div className="font-bold text-text-primary text-sm">{translateMarket(transaction.market_name, t)}</div>
            </div>
            ...
            <div className="bg-surface-bg p-4 rounded-xl border border-border-subtle space-y-1">
              <span className="text-text-secondary font-medium uppercase text-[10px]">{t("fpo.buyer")}</span>
              <div className="font-bold text-text-primary text-sm">{translateRole(transaction.buyer_name, t)}</div>
            </div>
          </div>
          ...
          <div className="bg-stone-900 text-stone-100 p-5 rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-bold uppercase tracking-wider text-agrigreen-500 flex items-center gap-1.5">
                <CreditCard className="w-4 h-4" />{t("transaction.demo_payment_controls")}</h4>
              <span className="text-[10px] text-text-secondary">{t("transaction.prototype_actions")}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              {transaction.payment_status === 'Pending' && (
                ...
                >
                  ...
                  <span>{t("transaction.mark_processing")}</span>
                </button>
              )}
              ...
              {transaction.payment_status === 'Paid' && (
                <div className="text-xs font-bold text-agrigreen-500 flex items-center gap-2">
                  ...
                  <span>{t("transaction.payment_completed")}</span>
                </div>
              )}
            </div>
          </div>
